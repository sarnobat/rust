...
#[cfg(not(has_antlr))]
compile_error!("ANTLR sources missing. Run `make antlr` to generate src/antlr/cronlexer.rs before building.");
...
fn main() {
    ...
    run_with_antlr(input);
}

#[cfg(has_antlr)]
fn run_with_antlr(input: String) {
